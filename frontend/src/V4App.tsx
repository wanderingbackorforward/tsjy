...
} function Shell({ active, children }: { active: string; children: React.ReactNode }) { const nav = [ ['/', '指挥总览', '地图/当前环'], ['/project-docs', '项目书', '工程依据'], ['/risk-replay', '风险复盘', '穿越窗口'], ['/monitoring-alerts', '监测异常', '阈值趋势'], ['/operation-diagnosis', '参数诊断', '组合异常'], ['/slurry-grouting', '泥水注浆', '沉降归因'], ['/segment-quality', '管片盾尾', '拼装缺陷'], ['/events', '事件闭环', '报警处置'], ['/data-import', '数据接入', '接口/映射'], ['/system-status', '系统状态', '数据质量'], ['/evidence', '证据链', '来源追溯'], ]; return ( <div className="v4-app"> <aside className="v4-side"> <div className="v4-brand"> <span>SHIELD TUNNEL ANALYTICS</span> <b>通苏嘉甬施工监测与盾构研判平台</b> </div> <nav> {nav.map(([href, title, sub]) => ( <a key={href} href={href} className={active === href ? 'active' : ''}> <b>{title}</b><span>{sub}</span> </a> ))} </nav> </aside> <main className="v4-main">{children}</main> </div> );
